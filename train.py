...
from nets.bisenet_training import get_lr_scheduler, set_optimizer_lr, weights_init
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path  = "./model_data/backbone_v2.pth"
    #-----------------------------------------------------#
    ...
    #   数据集路径
    #------------------------------#
    VOCdevkit_path  = 'F:/Project/Table-Recognition/TableRecognition/Structure-Recognition/Dataset/3Classes/Patch1024/'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    num_workers     = 4
    # ------------------------------------------------------#
    #   resume参数用于设置断点模型的路径，实现断点续训；不需要断点时将其设置为resume=""即可;
    #   断点续训功能一般用于Epoch数量比较大,例如1000时;
    # ------------------------------------------------------#
    resume = ""#./logs/best_epoch_weights.pt
    ...
    model = BiSeNetV1(n_classes=num_classes, aux_mode="train").train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
